...
export function Text({ id, value, as: AS }: TextProps) {
  ...
  const onContentBlur = useCallback((evt: ChangeEvent<HTMLHeadingElement>) => {
    ...
    if(previewElements) {
      ...
      const indexOfElementOnPreviewData = previewElements?.findIndex((data) => data.id === id)
      newPreviewElements[indexOfElementOnPreviewData].value = evt.currentTarget.innerHTML
