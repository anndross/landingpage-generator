...
import { Update } from "@/modules/editor/services/functions/update";
...
export type EditorStore = {
  ...
  updateCurrentElementStyles: (styles: CSSProperties) => void;
  updateCurrentElementSettings: (settings: any) => void;
};
...
export const useEditorStore = create<EditorStore>((set) => {
  return {
    ...
    // ATUALIZA O ELEMENTO OU A ÁRVORE
    setLayout: (data) =>
      set((state) => {
        const update = new Update(state.layout);
    ...
    // ATUALIZA AS FUNÇÕES DO EDITOR
    setEditorFunctions: (data) =>
      set((state) => ({
        editorFunctions: { ...state.editorFunctions, ...data },
      })),

    // ATUALIZA OS ESTILOS COM BASE NO ELEMENTO SELECIONADO PELA ÁRVORE
    updateCurrentElementStyles: (styles) =>
      set((state) => {
        const update = new Update(state.layout);

        const { currentElementToEdit, breakpoint } = state.editorFunctions;

        if (!currentElementToEdit || !breakpoint) return state;

        const data: EditorStoreLayoutType | ElementsType = {
    ...
    // ATUALIZA AS CONFIG COM BASE NO ELEMENTO SELECIONADO PELA ÁRVORE
    updateCurrentElementSettings: (settings) =>
      set((state) => {
        const update = new Update(state.layout);
